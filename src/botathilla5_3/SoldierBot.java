...
public class SoldierBot {
    ...
    private static MapLocation lockedRuinTarget = null;
    private static int lockedRuinRound = -999;
    private static int ruinTargetStartRound = -1;
    private static MapLocation ruinCooldownTarget = null;
    private static int ruinCooldownUntil = -1;

    private static MapLocation lockedSRPTarget = null;
    private static int lockedSRPRound = -999;
    private static int srpTargetStartRound = -1;
    private static MapLocation srpCooldownTarget = null;
    private static int srpCooldownUntil = -1;
    ...
    private static State greedyDecide(RobotController rc, MapLocation me,
        ...
        selectFocusTargets(rc, me, nearby, allies, enemies);
        ...
        double ruinFocus = ruinTargetStartRound > 0 ? Math.min(1.2, (round - ruinTargetStartRound) * 0.03) : 0.0;
        score[3] = GreedyCore.score(2.0, 0.4, 6.4 + ruinFocus - ruinDist * 0.02, 2.6, 0.8, 0.45, 1.0);
        dist[3] = ruinDist;
        tie[3] = GreedyCore.tieId(bestRuin);

        int srpDist = bestSRP == null ? 999 : me.distanceSquaredTo(bestSRP);
        feasible[4] = bestSRP != null;
        double srpFocus = srpTargetStartRound > 0 ? Math.min(1.0, (round - srpTargetStartRound) * 0.03) : 0.0;
        score[4] = GreedyCore.score(1.8, 0.2, 5.3 + srpFocus - srpDist * 0.03, 2.1, 0.4, 0.4, 1.0);
        dist[4] = srpDist;
    ...
    private static void selectFocusTargets(RobotController rc, MapLocation me,
            MapInfo[] nearby, RobotInfo[] allies, RobotInfo[] enemies) throws GameActionException {
        int round = rc.getRoundNum();

        MapLocation ruinCandidate = findBestRuin(rc, me, nearby, allies);
        if (shouldSkipCooldown(ruinCandidate, ruinCooldownTarget, ruinCooldownUntil, round)) {
            ruinCandidate = null;
        }
        if (lockedRuinTarget != null && (round - lockedRuinRound) <= 14 && !isRuinInvalid(rc, lockedRuinTarget)) {
            ruinCandidate = lockedRuinTarget;
        }
        if (ruinCandidate != null) {
            if (lockedRuinTarget == null || !lockedRuinTarget.equals(ruinCandidate)) {
                lockedRuinTarget = ruinCandidate;
                lockedRuinRound = round;
                ruinTargetStartRound = round;
            }
        } else if (lockedRuinTarget != null && isRuinInvalid(rc, lockedRuinTarget)) {
            lockedRuinTarget = null;
            ruinTargetStartRound = -1;
        }
        bestRuin = ruinCandidate;

        MapLocation srpCandidate = null;
        if (Messaging.towerCount >= SRP_TOWER_MIN && Messaging.countPaintTowers() > 0
                && enemies.length == 0 && rc.getChips() >= 120) {
            srpCandidate = findBestSRP(rc, me);
        }
        if (shouldSkipCooldown(srpCandidate, srpCooldownTarget, srpCooldownUntil, round)) {
            srpCandidate = null;
        }
        if (lockedSRPTarget != null && (round - lockedSRPRound) <= 12 && !isSRPInvalid(rc, lockedSRPTarget)) {
            srpCandidate = lockedSRPTarget;
        }
        if (srpCandidate != null) {
            if (lockedSRPTarget == null || !lockedSRPTarget.equals(srpCandidate)) {
                lockedSRPTarget = srpCandidate;
                lockedSRPRound = round;
                srpTargetStartRound = round;
            }
        } else if (lockedSRPTarget != null && isSRPInvalid(rc, lockedSRPTarget)) {
            lockedSRPTarget = null;
            srpTargetStartRound = -1;
        }
        bestSRP = srpCandidate;
    }

    private static boolean shouldSkipCooldown(MapLocation loc, MapLocation cooldownLoc, int cooldownUntil, int round) {
        return loc != null && cooldownLoc != null && round < cooldownUntil && loc.equals(cooldownLoc);
    }

    private static boolean isRuinInvalid(RobotController rc, MapLocation ruin) throws GameActionException {
        if (ruin == null) return true;
        if (!rc.canSenseLocation(ruin)) return false;
        RobotInfo robot = rc.senseRobotAtLocation(ruin);
        if (robot != null) return true;
        return !rc.senseMapInfo(ruin).hasRuin();
    }

    private static boolean isSRPInvalid(RobotController rc, MapLocation center) throws GameActionException {
        if (center == null) return true;
        if (!rc.onTheMap(center)) return true;
        if (!rc.canSenseLocation(center)) return false;
        return rc.senseMapInfo(center).isWall();
    }

    private static int countEnemyPaintAround(RobotController rc, MapLocation center, int radius) throws GameActionException {
        int c = 0;
        for (MapInfo info : rc.senseNearbyMapInfos(center, radius * radius + 2)) {
    ...
    private static void markRuinCooldown(RobotController rc, MapLocation ruin, int duration, String reason)
            throws GameActionException {
        if (ruin == null) return;
        int round = rc.getRoundNum();
        ruinCooldownTarget = ruin;
        ruinCooldownUntil = round + duration;
        lockedRuinTarget = null;
        ruinTargetStartRound = -1;
        reportSignalToTower(Messaging.encodeContestedRuin(ruin, reason.equals("PRESSURE") ? 3 : 1));
        GreedyCore.traceDecision(rc, "SOLDIER", "STATE", "ABORT_RUIN_" + reason, -25.0, -26.0, 0);
    }

    private static void markSRPCooldown(RobotController rc, MapLocation srp, int duration, String reason)
            throws GameActionException {
        if (srp == null) return;
        int round = rc.getRoundNum();
        srpCooldownTarget = srp;
        srpCooldownUntil = round + duration;
        lockedSRPTarget = null;
        srpTargetStartRound = -1;
        if ("PRESSURE".equals(reason)) {
            reportSignalToTower(Messaging.encodeRetreatBeacon(srp, 2));
        }
        GreedyCore.traceDecision(rc, "SOLDIER", "STATE", "ABORT_SRP_" + reason, -20.0, -21.0, 0);
    }

    private static void reportSignalToTower(int msgBytes) throws GameActionException {
        RobotController rc = RobotPlayer.rc;
        RobotInfo[] allies = rc.senseNearbyRobots(-1, RobotPlayer.myTeam);
        for (RobotInfo ally : allies) {
            if (ally.type.isTowerType()) {
                Messaging.trySendMessage(ally.location, msgBytes);
                break;
            }
        }
    }
    ...
    private static void buildTowerState(RobotController rc, MapLocation me,
        ...
        int round = rc.getRoundNum();
        if (ruinTargetStartRound > 0 && round - ruinTargetStartRound > 60) {
            markRuinCooldown(rc, bestRuin, 45, "TIMEOUT");
            state = State.EXPLORE;
            return;
        }
        if (countEnemyPaintAround(rc, bestRuin, 2) >= 8) {
            markRuinCooldown(rc, bestRuin, 35, "PRESSURE");
            state = State.EXPLORE;
            return;
        }
    ...
    private static void buildSRPState(RobotController rc, MapLocation me, MapInfo[] nearby) throws GameActionException {
        ...
        int round = rc.getRoundNum();
        if (srpTargetStartRound > 0 && round - srpTargetStartRound > 45) {
            markSRPCooldown(rc, bestSRP, 30, "TIMEOUT");
            state = State.EXPLORE;
            return;
        }
        if (countEnemyPaintAround(rc, bestSRP, 2) >= 6) {
            markSRPCooldown(rc, bestSRP, 30, "PRESSURE");
            state = State.EXPLORE;
            return;
        }
