...
public class SplasherBot {
    ...
    private static MapLocation bestSplashTarget(RobotController rc, MapLocation me, RobotInfo[] enemies) throws GameActionException {
        ...
        double pressure = localPressure(enemies);
        int round = rc.getRoundNum();
        int minNeed = minSplashValue(round, pressure);
        ...
        for (MapLocation loc : locs) {
            ...
            for (int[] d : offsets) {
                ...
                MapLocation tloc = loc.translate(d[0], d[1]);
                ...
                boolean isFrontier = (p.isEnemy() || p == PaintType.EMPTY) && isFrontierTile(rc, tloc);
                if (isFrontier) {
                    frontier++;
                    if (p.isEnemy()) contested++;
                }
            }

            double risk = 0.0;
            for (RobotInfo e : enemies) {
                int d = loc.distanceSquaredTo(e.location);
                if (e.type.isTowerType() && d <= e.type.actionRadiusSquared) {
                    risk += 2.2;
                } else if (!e.type.isTowerType() && d <= 8) {
                    risk += 0.45;
                }
            }

            double overpaint = allyPaint * (round >= 260 ? 1.2 : 0.8);
            double objective = frontier * 1.4 + contested * 1.1;
            double value = GreedyCore.score(
                    emptyPaint * 0.7 + frontier * 0.8,
                    enemyPaint * 2.0,
                    objective,
                    1.1,
                    pressure > 3.5 ? 0.7 : 0.4,
                    risk,
                    1.1 + overpaint);

            if (enemyPaint == 0 && frontier < 2) value -= 1.5;
        ...
        boolean allow = bestVal >= minNeed || (bestVal > 0.5 && (bestEnemyPaint >= 1 || bestFrontier >= 3));
        if (allow) {
    ...
    private static int minSplashValue(int round, double pressure) {
        int v = round < 120 ? 3 : round < 260 ? 2 : 1;
        if (pressure > 3.2) v--;
        return Math.max(0, v);
    }
