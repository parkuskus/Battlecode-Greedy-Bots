...
public class SoldierBot {
    ...
    private static final double RETREAT_ENTER = 0.05;
    private static final double RETREAT_EXIT = 0.16;
    ...
    private static State greedyDecide(RobotController rc, MapLocation me,
        ...
        boolean canExitRetreat = paintRatio > RETREAT_EXIT && pressure < 2.6;
        ...
        feasible[IDX_RETREAT] = !canExitRetreat && (retreatActive || paintRatio < RETREAT_ENTER || rc.getPaint() <= 2 || (pressure >= 5.0 && rc.getHealth() < 100));
        ...
        feasible[IDX_SKIRMISH] = bestSkirmish != null && paintRatio > 0.12;
        score[IDX_SKIRMISH] = GreedyCore.score(0.9, 1.5, 1.9, 0.6, countAlliedSoldiersNear(allies, bestSkirmish, 8) * 0.4, pressure * 1.1, 1.0);
        ...
        feasible[IDX_COMBAT_TOWER] = bestEnemyTower != null && rc.getHealth() > 65 && paintRatio > 0.36;
        score[IDX_COMBAT_TOWER] = GreedyCore.score(0.7, 0.8, 2.6, 0.4, 0.2, 1.8, twDist * 0.02);
        ...
        feasible[IDX_BUILD_TOWER] = bestRuin != null
                ...
                && !ruinTimedOut
                && !(bestRuinPressure > 3.4 && bestRuinEnemyPaint > 8);
        score[IDX_BUILD_TOWER] = GreedyCore.score(
                Math.max(1, 24 - ruinDist) * 0.22,
                bestRuinEnemyPaint * 0.35,
                7.4 - ruinDist * 0.025,
                3.1,
                Math.max(0, MAX_BUILDERS_PER_RUIN - bestRuinBuilders) * 1.6,
                bestRuinPressure * 0.85,
                1.0);
        ...
        feasible[IDX_BUILD_SRP] = bestSRP != null
                ...
                && rc.getChips() >= 100
                && pressure < 2.8;
        score[IDX_BUILD_SRP] = GreedyCore.score(Math.max(1, 16 - srpDist) * 0.35, 0.3, 5.6, 2.5, 0.5, pressure * 0.9, 0.9);
        ...
        score[IDX_EXPLORE] = GreedyCore.score(1.8, 0.4, 2.0, 0.9, 0.3, pressure * 0.35, 0.6);
    ...
    private static MapLocation findBestRuin(RobotController rc, MapLocation me,
        ...
        for (MapLocation ruin : ruins) {
            ...
            double score = GreedyCore.score(
                    Math.max(1, 26 - missing) * 0.35 + allyPaint * 0.2,
                    enemyPaint * 0.35,
                    9.2 - expectedTurns * 0.5,
                    2.8,
                    Math.max(0, MAX_BUILDERS_PER_RUIN - builders) * 1.4,
                    pressure * 0.9,
                    missing * 0.6 + me.distanceSquaredTo(ruin) * 0.05);
        ...
        for (int dx = -4; dx <= 4; dx += 4) {
            for (int dy = -4; dy <= 4; dy += 4) {
                ...
                double score = GreedyCore.score(
                        ...
                        0.25,
                        pressure * 0.9,
                        me.distanceSquaredTo(center) * 0.045);
                if (score > bestScore) {
                    bestScore = score;
                    best = center;
                }
