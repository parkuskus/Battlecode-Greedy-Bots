...
public class SplasherBot {
    ...
    private static State decide(RobotController rc, MapLocation me, MapInfo[] nearby,
        ...
        feasible[IDX_RETREAT] = retreatActive || paintRatio < 0.22 || pressure >= 2.8;
        ...
        feasible[IDX_PRESSURE] = splashTarget != null && rc.getPaint() >= 45;
        score[IDX_PRESSURE] = GreedyCore.score(3.2, 2.0, 3.1, 1.0, 0.4, pressure * 0.4, 1.3 + splashDist * 0.01);
    ...
    private static void pressureState(RobotController rc, MapLocation me, MapInfo[] nearby, RobotInfo[] enemies) throws GameActionException {
        ...
        if (rc.isMovementReady()) {
            MapLocation cluster = findPaintCluster(nearby);
            if (cluster != null && me.distanceSquaredTo(cluster) > 4) Nav.safeFuzzyMove(cluster, enemies);
    ...
    private static SplashScore scoreSplash(RobotController rc, MapLocation center, RobotInfo[] enemies) throws GameActionException {
        ...
        double value = GreedyCore.score(emptyPaint * 0.9 + frontier, enemyPaint * 1.6, frontier * 1.2, 0.9, 0.3, risk, 1.2 + allyPaint * 0.9);
