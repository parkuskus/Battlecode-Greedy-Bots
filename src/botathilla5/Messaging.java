 ...
 * 
 * v5 changes from v3:
 * - Cap relay messages to 3 per tower interaction (save message quota)
 * - nearestRefillTower: prefer paint towers but accept ANY tower type
 */
public class Messaging {
    ...
    static void init() { towerCount = 0; needMopperAt = null; }
    ...
    static void readMessages() throws GameActionException {
        ...
        int from = Math.max(0, rc.getRoundNum() - 2);
    ...
    static void registerTower(MapLocation loc, UnitType type) {
        for (int i = 0; i < towerCount; i++)
            if (knownTowers[i].equals(loc)) { knownTypes[i] = type; return; }
        if (towerCount < knownTowers.length) {
            knownTowers[towerCount] = loc; knownTypes[towerCount] = type; towerCount++;
        }
    }
    ...
    static MapLocation nearestRefillTower(MapLocation from) {
        MapLocation bestPaint = null; int bpd = Integer.MAX_VALUE;
        MapLocation bestAny = null; int bad = Integer.MAX_VALUE;
        for (int i = 0; i < towerCount; i++) {
            int d = from.distanceSquaredTo(knownTowers[i]);
            if (knownTypes[i] != null && knownTypes[i].getBaseType() == UnitType.LEVEL_ONE_PAINT_TOWER) {
                if (d < bpd) { bpd = d; bestPaint = knownTowers[i]; }
            }
            if (d < bad) { bad = d; bestAny = knownTowers[i]; }
        }
    ...
    /** Relay tower info to a nearby tower — capped at 3 messages to save quota. */
            ...
            if (ally.type.isTowerType() && rc.canSendMessage(ally.location)) {
                ...
                for (int i = 0; i < towerCount && sent < 3; i++) {
                    if (rc.canSendMessage(ally.location)) {
                        rc.sendMessage(ally.location, encodeTowerBuilt(knownTowers[i], knownTypes[i]));
                        sent++;
                    }
                }
                break;
