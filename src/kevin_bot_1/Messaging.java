 ...
 *
 * Format (high → low bits):
 *   [31..29] 3-bit code
 *   [28..23] x coordinate (6 bits, max 63)
 *   [22..17] y coordinate (6 bits, max 63)
 *   [16..15] payload (2 bits — tower type, etc.)
 *   [14..0]  unused / future
 *
 * Codes:
...
public class Messaging {
    ...
    /* ---- cached friendly tower list (shared by all units) ---- */
    ...
    static void readMessages() throws GameActionException {
        ...
        for (Message msg : rc.readMessages(from)) {
            ...
            switch (code) {
                ...
                case CODE_ENEMY_TOWER -> { /* store if needed */ }
            }
        }
    }
    ...
    /** Relay our known tower list to a nearby ally tower. */
    static void relayToNearbyTower(RobotInfo[] allies) throws GameActionException {
        ...
        for (RobotInfo ally : allies) {
            if (ally.type.isTowerType() && rc.canSendMessage(ally.location)) {
                ...
                break; // only relay to one tower per turn to save bytecodes
            }
        }
    }
}
