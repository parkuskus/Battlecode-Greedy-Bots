...
public class SoldierBot {
    ...
    private static State greedyDecide(RobotController rc, MapLocation me,
        ...
        // Refill — hanya jika sangat rendah DAN ada tower nearby
        if ((state != State.REFILL && paintRatio < REFILL_LOW)
         || (state == State.REFILL && paintRatio < REFILL_HIGH)) {
            // v5: use nearestRefillTower (prefers paint, accepts any)
        ...
        // Combat — hanya soldier yang boleh serang tower musuh!
        // v5: threshold raised from 0.15 to 0.30
        RobotInfo enemyTower = Nav.nearestEnemyTower(enemies);
        if (enemyTower != null && rc.getHealth() > 40 && paintRatio > 0.30) {
            return State.COMBAT;
        }

        // Build tower — PRIORITAS UTAMA: bangun sebanyak mungkin
        bestRuin = findBestRuin(rc, me, nearby, allies);
        if (bestRuin != null && rc.getChips() >= 200) return State.BUILD_TOWER;

        // Build SRP — jika tower sudah cukup banyak & aman
        // v5: added chip guard (need >= 100 chips buffer)
